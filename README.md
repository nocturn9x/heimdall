...
You can also run `make zen2` to build a modern version of Heimdall with a more generic instruction set (AVX2 support is still required here,
...
architecture the compile was done on (and is what you want for releases/sharing the result). For slightly older CPUs, try `make modern` instead. 
For (very) old CPUs without AVX2 support, run `make legacy` (note that the resulting executable will be significantly slower though). In every case,
...
**P.S**: There's also equivalent `windows_*` targets for all builds that are used for cross compiling for windows
from Linux using zigcc and MinGW (you probably don't want these)
...
and 8 output buckets, and is commonly represented as (704x16hm->1536)x2->1x8
