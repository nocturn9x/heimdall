...
### Requirements
- The Nim compiler (2.2.2 or newer). See [here](https://codeberg.org/janAkali/grabnim) for more details
...
For a quick regression pass with dependencies and network weights installed, run
`make dev` followed by `python -m unittest discover -s tests -p 'test_*.py'`.
See [AGENTS.md](AGENTS.md#tests-and-benchmarks) for focused tests, the full test
suite, and performance comparisons.
...
Progress updates cover chunk loading/splitting, live per-position worker progress, and final joining, with throughput, elapsed time, and ETA. Loaded chunks are divided into position-balanced batches and scheduled dynamically across workers. Updates reuse one terminal line when stdout is a TTY and remain newline-delimited when redirected to a file. Runs with `--limit` use game-count progress; unlimited runs estimate progress from the input byte offset without pre-scanning the file.
