...
You can also run `make zen2` to build a modern version of Heimdall for slightly older CPUs that will run on more CPUs than just yours. For even slightly older CPUs,
and a more generic binary, try `make modern` instead. For (very) old CPUs without AVX2 support, run `make legacy`. In every case, the resulting executable will be
located at `bin/$(EXE)` (`bin/heimdall` by default).
