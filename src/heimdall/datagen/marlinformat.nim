...
proc encodePieces(position: Position): string =
    ...
    for sq in occupancy:
        ...
        if sq == position.castlingAvailability[piece.color][false] or sq == position.castlingAvailability[piece.color][true]:
...
func encodeStmAndEp(position: Position): string =
    ...
    let epTarget = if position.enPassantSquare != nullSquare(): position.enPassantSquare else: nullSquare()
...
proc fromMarlinformat*(data: string): MarlinFormatRecord =
    ...
    for i, sq in occupancy:
        ...
        if pieceNum == 6:
            ...
            if kingSeen[color]:
                result.position.castlingAvailability[color][false] = sq
            else:
                result.position.castlingAvailability[color][true] = sq
    ...
    let epSquare = Square(stmAndEpSquare and 0b01111111)
    let stm = stmAndEpSquare shr 7

    result.position.sideToMove = if stm == 0: White else: Black
    result.position.enPassantSquare = epSquare
