...
proc search(self: var SearchManager, depth, ply: int, alpha, beta: Score, isPV, root: static bool, cutNode: bool, excluded=nullMove()): Score {.discardable, gcsafe.} =
    ...
    when not isPV:
        ...
        if depth <= RAZORING_DEPTH_LIMIT and abs(staticEval) < 2000 and staticEval + self.parameters.razoringMargin * depth <= alpha:
            ...
            if razorScore + self.parameters.razoringMargin * depth <= alpha:
                return razorScore
