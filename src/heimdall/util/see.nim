...
func gain(parameters: SearchParameters, position: Position, move: Move): int =
    ...
    if move.isPromotion():
        result += parameters.getStaticPieceScore(move.promotionToPiece()) - parameters.getStaticPieceScore(Pawn)
...
proc see*(parameters: SearchParameters, position: Position, move: Move, threshold: int): bool =
    ...
    var next = if move.isPromotion(): move.promotionToPiece() else: position.getPiece(move.startSquare).kind
