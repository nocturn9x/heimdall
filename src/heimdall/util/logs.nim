    ...
    for i, move in line:
        ...
        if move.isCastling() and not chess960:
            ...
            if move.targetSquare < move.startSquare:
                move = createMove(move.startSquare, makeSquare(rankFromSquare(move.targetSquare), fileFromSquare(move.targetSquare) + 2), Castle)
            else:
                move = createMove(move.startSquare, makeSquare(rankFromSquare(move.targetSquare), fileFromSquare(move.targetSquare) - 1), Castle)
