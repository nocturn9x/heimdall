type
  M512i* {.importc: "__m512i", header: "immintrin.h", bycopy.} = object

{.push header: "immintrin.h".}

func mm512_add_epi32*(a, b: M512i): M512i {.importc: "_mm512_add_epi32".}

func mm512_load_si512(p: ptr M512i): M512i {.importc: "_mm512_load_si512".}

func mm512_store_si512*(a: pointer, b: M512i) {.importc: "_mm512_store_si512".}

template mm512_load_si512*(p: pointer): M512i =
  mm512_load_si512(cast[ptr M512i](p))

func mm512_add_epi16*(a, b: M512i): M512i {.importc: "_mm512_add_epi16".}

func mm512_sub_epi16*(a, b: M512i): M512i {.importc: "_mm512_sub_epi16".}

func mm512_madd_epi16*(a, b: M512i): M512i {.importc: "_mm512_madd_epi16".}

func mm512_max_epi16*(a, b: M512i): M512i {.importc: "_mm512_max_epi16".}

func mm512_min_epi16*(a, b: M512i): M512i {.importc: "_mm512_min_epi16".}

func mm512_mullo_epi16*(a, b: M512i): M512i {.importc: "_mm512_mullo_epi16".}

func mm512_mulhi_epi16*(a, b: M512i): M512i {.importc: "_mm512_mulhi_epi16".}

func mm512_maddubs_epi16*(a, b: M512i): M512i {.importc: "_mm512_maddubs_epi16".}

func mm512_dpbusd_epi32*(acc, a, b: M512i): M512i {.importc: "_mm512_dpbusd_epi32".}

func mm512_max_epi32*(a, b: M512i): M512i {.importc: "_mm512_max_epi32".}

func mm512_min_epi32*(a, b: M512i): M512i {.importc: "_mm512_min_epi32".}

func mm512_mullo_epi32*(a, b: M512i): M512i {.importc: "_mm512_mullo_epi32".}

func mm512_set1_epi32*(a: int32 | uint32): M512i {.importc: "_mm512_set1_epi32".}

func mm512_slli_epi32*(vec: M512i, shift: int32 | uint32): M512i {.importc: "_mm512_slli_epi32".}

func mm512_srli_epi32*(vec: M512i, shift: int32 | uint32): M512i {.importc: "_mm512_srli_epi32".}

func mm512_srai_epi32*(vec: M512i, shift: int32 | uint32): M512i {.importc: "_mm512_srai_epi32".}

func mm512_set1_epi16*(a: int16 | uint16): M512i {.importc: "_mm512_set1_epi16".}

func mm512_setzero_si512*(): M512i {.importc: "_mm512_setzero_si512".}

func mm512_reduce_add_epi32*(a: M512i): int32 {.importc: "_mm512_reduce_add_epi32".}

func mm512_slli_epi16*(vec: M512i, shift: int32 | uint32): M512i {.importc: "_mm512_slli_epi16".}

func mm512_srli_epi16*(vec: M512i, shift: int32 | uint32): M512i {.importc: "_mm512_srli_epi16".}

func mm512_srai_epi16*(vec: M512i, shift: int32 | uint32): M512i {.importc: "_mm512_srai_epi16".}

func mm512_packus_epi16*(vec0, vec1: M512i): M512i {.importc: "_mm512_packus_epi16".}

func mm512_permutexvar_epi64*(vec0, vec1: M512i): M512i {.importc: "_mm512_permutexvar_epi64".}

func mm512_setr_epi64*(e0, e1, e2, e3, e4, e5, e6, e7: uint64): M512i {.importc: "_mm512_setr_epi64".}